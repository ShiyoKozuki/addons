...
local function GetBuffs(index)
    ...
    --Get your own buffs, party index is 0 = self
    if (index == 0) then
    ...
    local playerIndex = partyMgr:GetMemberTargetIndex(index);

    -- Get buffs for anyone besides yourself that's sent via your custom packet..
    local packetData = buffTable[playerIndex];
    if packetData then
        for _,entry in ipairs(packetData) do
            buffs:append(entry.Id);
        end
        return buffs;
    end
    local playerId = partyMgr:GetMemberServerId(index)

    --Get buffs for someone in party that isn't yourself and isn't in custom packet
    for i = 0,4 do
...
function member:Render()
    ...
    local maxBuffs = settings.max_buffs
    ...
    -- Limit the number of buffs to 'maxBuffs'
    local limitedBuffs = sorted:slice(1, maxBuffs)

    -- Calculate the starting X position based on limited buffs
    local firstBuffPositionX = self.PositionX - (#limitedBuffs * settings.size);
    local buffPositionX = firstBuffPositionX;

    -- Draw the member buffs (limited to 'maxBuffs')
    for i, buff in ipairs(limitedBuffs) do
        renderer:Draw(GetStatusIconById(buff), buffPositionX, self.PositionY, settings.size / 32);
        buffPositionX = buffPositionX + settings.size; -- Increment X only for valid buffs
    end
