addon.name      = 'AbilityAnimation';
addon.author    = 'Shiyo';
addon.version   = '1.0.0.0';
addon.desc      = 'Prints out ability animation IDs';
...
ashita.events.register('packet_in', 'HandleIncomingPacket', function (e)
    ...
    if (e.id == 0x28) then
      local actionPacket = ParseActionPacket(e);
        -- Ability finsh
        if (actionPacket.Type == 6) then
            for _,target in ipairs(actionPacket.Targets) do
                for _,action in ipairs(target.Actions) do
                    print('Action ID: ' .. actionPacket.Id)
                    print('Action animation: ' .. action.Animation) -- Ability animation ID
                end
            end
        end
      end
