...
local function ParseActionPacket(e)
    ...
    PendingActionPacket.ActionType = UnpackBits(4);
    PendingActionPacket.ActionId = UnpackBits(32);
    --Unknown 32 bits
...
gResonance = T{};
gResonance.Resonation = T{};

function gResonance:Destroy()
    ashita.events.unregister('packet_in', 'packet_in_resonance_cb');
end

function gResonance:GetResonance(targetIndex)
    return self.Resonation[targetIndex];
end

function gResonance:GetSkillchain(resonation, weaponskillId)
    local wsAttributes = weaponskillResonationMap[weaponskillId];
    if not wsAttributes then
        return Resonation.None;
    end

    for _,sc in ipairs(possibleSkillchains)
...
function gResonance:Initialize()
    ashita.events.register('packet_in', 'packet_in_resonance_cb', function (e)
        if (e.id == 0x28) then
            local action = ParseActionPacket(e);
            if (action.ActionType == 3) then
                self:HandleWeaponskill(action);
            elseif (action.ActionType == 4) then
                self:HandleSpell(action);
            elseif (action.ActionType == 6) then
                self:HandleJobAbility(action);
            end
        end
    end);
end

function gResonance:HandleJobAbility(action)
    if (action.ActionId == 94) then
        chainAffinityMap[action.UserId] = os.clock() + 30;
    elseif (action.ActionId == 317) then
        immanenceMap[action.UserId] = os.clock() + 60;
    end
end

function gResonance:HandleSpell(action)
    for _,target in pairs(action.Targets) do
        local targetIndex = bit.band(target.Id, 0x7FF);
        if (gEntityManager:GetServerId(targetIndex) == target.Id) then
            for _,action in pairs(target.Actions) do
                local skillchain = skillchainMessageIds[action.Message];
                if skillchain then
                    local resonation = self.Resonation[targetIndex];
                    if resonation and ((os.clock() + 1) > resonation.WindowOpen) and ((os.clock() - 1) < resonation.WindowClose) then
                        ...
                        resonation.WindowOpen = os.clock() + 3.5;
                        resonation.WindowClose = os.clock() + (9.8 - resonation.Depth);
                        self.Resonation[targetIndex] = resonation;
                    end
                else
                    local immanence = immanenceMap[action.UserId];
                    if immanence and (os.clock() < immanence) then
                        local elements = immanenceResonationMap[action.ActionId];
                        if elements then
                            local resonation = {};
                            resonation.Depth = 1;
                            ...
                            resonation.WindowClose = os.clock() + (9.8 - resonation.Depth);
                            self.Resonation[targetIndex] = resonation;
                            immanenceMap[action.UserId] = nil;
                        end
                    end

                    local chainAffinity = chainAffinityMap[action.UserId];
                    if chainAffinity and (os.clock() < chainAffinity) then
                        local elements = chainAffinityResonationMap[action.ActionId];
                        if elements then
                            local resonation = {};
                            resonation.Depth = 1;
                            resonation.Attributes = elements;
                            ...
                            resonation.WindowClose = os.clock() + (9.8 - resonation.Depth);
                            self.Resonation[targetIndex] = resonation;
                            chainAffinityMap[action.UserId] = nil;
                        end
                    end          
                end
            end
...
function gResonance:HandleWeaponskill(action)    
    for _,target in pairs(action.Targets) do
        local targetIndex = bit.band(target.Id, 0x7FF);
        if (gEntityManager:GetServerId(targetIndex) == target.Id) then
            for _,action in pairs(target.Actions) do
                ...
                if skillchain == Resonation.None then
                    self.Resonation[targetIndex] = nil;
                elseif skillchain then
                    ...
                    local attributes = weaponskillResonationMap[action.ActionId];
                    if attributes then
                        resonation = {};
                        resonation.Depth = 0;
