...
targetbar.DrawWindow = function(settings)
    ...
    if (imgui.Begin('TargetBar', true, windowFlags)) then
        ...
        -- Determine the correct buffs to show based on the target
        if targetEntity == playerEnt then
            -- Get buffs for the player
            buffIds = player:GetBuffs()
        elseif IsMemberOfParty(targetIndex) then
            -- Get buffs for party members using status handler
            buffIds = statusHandler.get_member_status(playerTarget:GetServerId(0))
        else
            -- Use my GetEntitiesBuffs() to get status effect info from my custom packet
            local memMgr = AshitaCore:GetMemoryManager();
            local targetMgr = memMgr:GetTarget();
            local targetIndex = targetMgr:GetTargetIndex(0);
            local buffs = GetEntitiesBuffs(targetIndex)

            for _, buff in ipairs(buffs) do
                if buff.Id and buff.Id ~= -1 then  -- Ensure buff.Id is valid and not -1
                    if (buff.Id <= 776) then -- Exclude prowess and custom status effects
        ...
        -- Draw a line to separate the status icons
        imgui.NewLine()
